...
#### 特点
 1. 使用了 Disruptor 框架，其是一个高性能的、无锁的环形数组结构，主要用于实现日志框架、消息中间件等场景的高性能异步处理。它被设计出来是为了替代某些使用队列的场景，并提高吞吐量。在该自研网关中是用来接受客户端请求，然后经过消费者消费转发到具体后端服务。经过jmeter测压，其与传统的阻塞队列（如Java的 ArrayBlockingQueue）相比，吞吐量可以提高10倍甚至更多。
 2. 引入了Caffeine缓存库，具有灵活的缓存策略、独特的刷新机制，同时也是无锁设计的。
 3. 通过 grafana + prometheus 展示流量（请求）的相关属性，便于对流量进行分析，从而调整相关限流参数等等。
#### 功能（目前）
 + 路由
 + 负载均衡
 + 限流
